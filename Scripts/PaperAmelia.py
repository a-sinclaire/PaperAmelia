...
# load in all assets and append to df
sprites = [None] * df.shape[0]
# for i in range(len(sprites)):
#     s = pygame.image.load('../Assets/' + str(df.iloc[i]['img_name']))
#     s = scale_img(s, SCALE)
#     sprites[i] = s
#     print("Loaded ", i, " item(s)...")
df = df.assign(Sprite=sprites)


def shuffle(outfit_status=None):
    global status, num_layers, num_items
...
def minor_shuffle(z, k, s=None):
    global status, num_items
    if locked[z]:
        return
...
def major_optimize(axis, max_iterations, max=True):
    global status
    ...
    for i in range(max_iterations):
        ...
        new_outfit = shuffle(deepcopy(status))
        stats = calc_stats(new_outfit)
...
def multi_opt(axis_array, iterations):
    ...
    for i in range(round(iterations)):
        ...
        minor_optimize(index_array[ind[0]], 1, axis_array_maxs[ind[0]], rand=True)


def minor_optimize(axis, iterations, max=True, rand=False):
    global status
    ...
    for i in range(iterations):
        ...
        for j in range(num_items[layer_op] + 1):
            ...
            new_outfit = minor_shuffle(layer_op, item_op, deepcopy(status))
            #
            # display(new_outfit)
            # draw_overlay()
            # pygame.draw.circle(screen, (0, 0, 255), (WIDTH - 20, 40), 10)
...
def calc_stats(outfit_status=None):
    global status
    working_status = status
    if outfit_status is not None:
        working_status = outfit_status
...
def display(outfit_status=None):
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_m:
                major_optimize(OP_AXIS, 1000, OP_AXIS_MAX)
            if event.key == pygame.K_n:
                minor_optimize(OP_AXIS, 300, OP_AXIS_MAX)
                # multi_opt(None, 100)
